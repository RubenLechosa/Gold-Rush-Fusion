            ...
            if($badge["value"] > 0) {
                ...
                if($user->save()) {
                    if($user_badge = Badges::where("id_user", "=", $request["id_user"])->where("type", "=", $badge["name"])->first()) {
                        ...
                        $user_badge->level = ($badge["value"] >= 500 ? 1 : 0);
                        $user_badge->id_user = $request["id_user"];
                        $user_badge->type = $badge["name"];
                        $user_badge->actualExp = ($badge["value"] >= 500 ? 0 : $badge["value"]);
                        $user_badge->expToNextLvl = ($badge["value"] >= 500 ? 1000 : 500);
